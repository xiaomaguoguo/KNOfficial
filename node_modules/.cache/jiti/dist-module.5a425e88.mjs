"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var _promises = _interopRequireDefault(await jitiImport("node:fs/promises"));
var _kit = await jitiImport("@nuxt/kit");
var _defu = await jitiImport("defu");
var _kit2 = await jitiImport("nuxt-site-config/kit");
var _pathe = await jitiImport("pathe");
var _pkgTypes = await jitiImport("pkg-types");
var _ufo = await jitiImport("ufo");
var _robotsDDgLOylu = await jitiImport("./shared/robots.DDgLOylu.mjs");
var _nodeFs = await jitiImport("node:fs");
var _stdEnv = await jitiImport("std-env");
var _util = await jitiImport("../dist/runtime/util.js");
var _const = await jitiImport("../dist/runtime/const.js");
var _nitro = await jitiImport("../dist/runtime/server/nitro.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}

const DEVTOOLS_UI_ROUTE = "/__nuxt-robots";
const DEVTOOLS_UI_LOCAL_PORT = 3030;
function setupDevToolsUI(options, resolve, nuxt = (0, _kit.useNuxt)()) {
...
function detectTarget(options = {}) {
  return options?.static ? autodetectableStaticProviders[_stdEnv.provider] : autodetectableProviders[_stdEnv.provider];
}
function resolveNitroPreset(nitroConfig) {
  if (_stdEnv.provider === "stackblitz" || _stdEnv.provider === "codesandbox")
  return _stdEnv.provider;
  let preset;
  if (nitroConfig && nitroConfig?.preset)
  preset = nitroConfig.preset;
  if (!preset)
  preset = _stdEnv.env.NITRO_PRESET || detectTarget() || "node-server";
  return preset.replace("_", "-");
}
async function getNuxtModuleOptions(module, nuxt = (0, _kit.useNuxt)()) {
  const moduleMeta = { name: module } || {};
  const { nuxtModule } = await (0, _kit.loadNuxtModuleInstance)(module, nuxt);
  let moduleEntry;
...
function isNuxtGenerate(nuxt = (0, _kit.useNuxt)()) {
...
const logger = (0, _kit.useLogger)("@nuxt/robots");
...
function splitPathForI18nLocales(path, autoI18n) {
  ...
  if (!path || (0, _util.isInternalRoute)(path))
  return path;
...
async function resolveI18nConfig() {
  ...
  if ((0, _kit.hasNuxtModule)("@nuxtjs/i18n")) {
    const i18nVersion = await (0, _kit.getNuxtModuleVersion)("@nuxtjs/i18n");
    if (!(await (0, _kit.hasNuxtModuleCompatibility)("@nuxtjs/i18n", ">=8")))
    ...
    normalisedLocales = (0, _util.mergeOnKey)((nuxtI18nConfig.locales || []).map((locale) => typeof locale === "string" ? { code: locale } : locale), "code");
...
const _module2 = exports.default = (0, _kit.defineNuxtModule)({
  meta: {
    name: "@nuxtjs/robots",
    compatibility: {
      nuxt: ">=3.6.1"
  ...
  defaults: {
    ...
    robotsEnabledValue: _const.ROBOT_DIRECTIVE_VALUES.enabled,
    robotsDisabledValue: _const.ROBOT_DIRECTIVE_VALUES.disabled,
    disallowNonIndexableRoutes: false,
    robotsTxt: true,
    botDetection: true
  },
  async setup(config, nuxt) {
    const { resolve } = (0, _kit.createResolver)("file:///Users/knothing/Documents/ASLY/AI-Project/aiknothing/node_modules/@nuxtjs/robots/dist/module.mjs");
    const { version } = await (0, _pkgTypes.readPackageJSON)(resolve("../package.json"));
    logger.level = config.debug || nuxt.options.debug ? 4 : 3;
    if (config.enabled === false) {
      logger.debug("The module is disabled, skipping setup.");
      (0, _kit.addImports)({ name: "useRobotsRule", from: resolve(`./runtime/app/composables/mock`) });
    ...
    if (config.blockAiBots) {
      config.groups.push({
        userAgent: _robotsDDgLOylu.A,
        comment: ["Block AI Crawlers"],
        disallow: ["/"]
      });
    }
    await (0, _kit2.installNuxtSiteConfig)();
    if (config.metaTag)
    (0, _kit.addPlugin)({ mode: "server", src: resolve("./runtime/app/plugins/robot-meta.server") });
    if (config.robotsTxt && config.mergeWithRobotsTxtPath !== false) {
      ...
      if (config.mergeWithRobotsTxtPath === true) {
        for (const path of validPaths) {
          robotsTxt = await _promises.default.readFile(path, { encoding: "utf-8" }).catch(() => false);
          if (robotsTxt) {
        ...
        if (!(await _promises.default.stat(customPath).catch(() => false))) {
          ...
          robotsTxt = await _promises.default.readFile(customPath, { encoding: "utf-8" });
        }
      }
      if (typeof robotsTxt === "string") {
        const path = (0, _pathe.relative)(nuxt.options.rootDir, usingRobotsTxtPath);
        logger.debug(`A robots.txt file was found at \`./${path}\`, merging config.`);
        const parsedRobotsTxt = (0, _util.parseRobotsTxt)(robotsTxt);
        const { errors } = (0, _util.validateRobots)(parsedRobotsTxt);
        if (errors.length > 0) {
        ...
        const wildCardGroups = parsedRobotsTxt.groups.filter((group) => (0, _util.asArray)(group.userAgent).includes("*"));
        if (wildCardGroups.some((group) => (0, _util.asArray)(group.disallow).includes("/"))) {
        ...
        if (host) {
          (0, _kit2.updateSiteConfig)({
            _context: usingRobotsTxtPath,
            url: host
          });
        }
        config.sitemap = [... /* @__PURE__ */new Set([...(0, _util.asArray)(config.sitemap), ...parsedRobotsTxt.sitemaps])];
        if (usingRobotsTxtPath === publicRobotsTxtPath) {
          await _promises.default.rename(usingRobotsTxtPath, resolve(nuxt.options.rootDir, nuxt.options.dir.public, "_robots.txt"));
          ...
          for (const path2 of validPaths) {
            ...
            extraPaths.push(` - ./${(0, _pathe.relative)(nuxt.options.rootDir, path2)}`);
          }
...
`);
    ...
    const usingNuxtContent = (0, _kit.hasNuxtModule)("@nuxt/content");
    const isNuxtContentV3 = usingNuxtContent && (await (0, _kit.hasNuxtModuleCompatibility)("@nuxt/content", "^3"));
    let isNuxtContentV2 = usingNuxtContent && (await (0, _kit.hasNuxtModuleCompatibility)("@nuxt/content", "^2"));
    if (isNuxtContentV3) {
        ...
        if (typeof ctx.content.robots !== "undefined") {
          ...
          if (typeof rule === "boolean") {
            ...
            for (const [key, value] of Object.entries(rule)) {
              ...
              continue;
              if (key in _const.ROBOT_DIRECTIVE_VALUES && typeof value === "boolean" && value) {
                directives.push(_const.ROBOT_DIRECTIVE_VALUES[key]);
              } else if (key === "max-image-preview" && typeof value === "string") {
                directives.push((0, _const.formatMaxImagePreview)(value));
              } else if (key === "max-snippet" && typeof value === "number") {
                directives.push((0, _const.formatMaxSnippet)(value));
              } else if (key === "max-video-preview" && typeof value === "number") {
                directives.push((0, _const.formatMaxVideoPreview)(value));
              }
            }
    ...
    nuxt.hook("modules:done", async () => {
      config.sitemap = (0, _util.asArray)(config.sitemap);
      config.disallow = (0, _util.asArray)(config.disallow);
      config.allow = (0, _util.asArray)(config.allow);
      config.groups = config.groups.map(_util.normalizeGroup);
      ...
      if (config.header) {
        const noIndexPaths = [(0, _ufo.withoutTrailingSlash)(nuxt.options.app.buildAssetsDir), `${nuxt.options.app.buildAssetsDir}**`];
        for (const path of noIndexPaths) {
          nuxt.options.routeRules[path] = (0, _defu.defu)({
            robots: "noindex"
          }, nuxt.options.routeRules[path]);
        }
        Object.entries(nuxt.options.routeRules).forEach(([route, rules]) => {
          const robotRule = (0, _nitro.normaliseRobotsRouteRule)(rules);
          if (robotRule && !robotRule.allow && robotRule.rule) {
            nuxt.options.routeRules[route] = (0, _defu.defu)({
              headers: {
      ...
      if (config.disallowNonIndexableRoutes) {
        Object.entries(nuxt.options.routeRules || {}).forEach(([route, rules]) => {
          ...
          const robotsRule = (0, _nitro.normaliseRobotsRouteRule)(rules);
      ...
      if (resolvedAutoI18n && resolvedAutoI18n.locales && resolvedAutoI18n.strategy !== "no_prefix") {
        ...
        for (const group of config.groups.filter((g) => !g._skipI18n)) {
          group.allow = (0, _util.asArray)(group.allow || []).map((path) => splitPathForI18nLocales(path, i18n)).flat();
          group.disallow = (0, _util.asArray)(group.disallow || []).map((path) => splitPathForI18nLocales(path, i18n)).flat();
        }
      }
      const groups = config.groups.map(_util.normalizeGroup);
      const pathsToCheck = [
      (0, _ufo.withoutTrailingSlash)(nuxt.options.app.buildAssetsDir),
      ...
      for (const p of pathsToCheck) {
        if (groups.some((g) => g.disallow.includes(p))) {
          logger.warn(`You have disallowed robots accessing \`${(0, _ufo.withTrailingSlash)(p)}**\`, this may prevent your site from being indexed correctly.`);
        }
      }
    ...
    (0, _kit.addTypeTemplate)({
      ...
      getContents: (data) => {
        const typesPath = (0, _pathe.relative)(resolve(data.nuxt.options.rootDir, data.nuxt.options.buildDir, "module"), resolve("runtime/types"));
...
`;
    ...
    (0, _kit.addServerPlugin)(resolve("./runtime/server/plugins/initContext"));
    if (isNuxtContentV2) {
      (0, _kit.addServerHandler)({
