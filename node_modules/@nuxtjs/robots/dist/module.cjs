...
const util_js = require('../dist/runtime/util.js');
const const_js = require('../dist/runtime/const.js');
const nitro_js = require('../dist/runtime/server/nitro.js');
...
function splitPathForI18nLocales(path, autoI18n) {
  ...
  if (!path || util_js.isInternalRoute(path))
    return path;
...
async function resolveI18nConfig() {
  ...
  if (kit.hasNuxtModule("@nuxtjs/i18n")) {
    ...
    normalisedLocales = util_js.mergeOnKey((nuxtI18nConfig.locales || []).map((locale) => typeof locale === "string" ? { code: locale } : locale), "code");
...
const module$1 = kit.defineNuxtModule({
  ...
  defaults: {
    ...
    robotsEnabledValue: const_js.ROBOT_DIRECTIVE_VALUES.enabled,
    robotsDisabledValue: const_js.ROBOT_DIRECTIVE_VALUES.disabled,
  ...
  async setup(config, nuxt) {
    ...
    if (config.blockNonSeoBots) {
      config.groups.push({
        userAgent: _const.NonHelpfulBots,
    ...
    if (config.robotsTxt && config.mergeWithRobotsTxtPath !== false) {
      ...
      if (typeof robotsTxt === "string") {
        ...
        const parsedRobotsTxt = util_js.parseRobotsTxt(robotsTxt);
        const { errors } = util_js.validateRobots(parsedRobotsTxt);
        if (errors.length > 0) {
        ...
        const wildCardGroups = parsedRobotsTxt.groups.filter((group) => util_js.asArray(group.userAgent).includes("*"));
        if (wildCardGroups.some((group) => util_js.asArray(group.disallow).includes("/"))) {
        ...
        config.sitemap = [.../* @__PURE__ */ new Set([...util_js.asArray(config.sitemap), ...parsedRobotsTxt.sitemaps])];
...
`);
          ...
          if (typeof rule === "boolean") {
            ...
            for (const [key, value] of Object.entries(rule)) {
              if (value === false || value === null || value === void 0)
                continue;
              if (key in const_js.ROBOT_DIRECTIVE_VALUES && typeof value === "boolean" && value) {
                directives.push(const_js.ROBOT_DIRECTIVE_VALUES[key]);
              } else if (key === "max-image-preview" && typeof value === "string") {
                directives.push(const_js.formatMaxImagePreview(value));
              } else if (key === "max-snippet" && typeof value === "number") {
                directives.push(const_js.formatMaxSnippet(value));
              } else if (key === "max-video-preview" && typeof value === "number") {
                directives.push(const_js.formatMaxVideoPreview(value));
              }
            }
    ...
    nuxt.hook("modules:done", async () => {
      config.sitemap = util_js.asArray(config.sitemap);
      config.disallow = util_js.asArray(config.disallow);
      config.allow = util_js.asArray(config.allow);
      config.groups = config.groups.map(util_js.normalizeGroup);
      ...
      if (config.header) {
        ...
        Object.entries(nuxt.options.routeRules).forEach(([route, rules]) => {
          const robotRule = nitro_js.normaliseRobotsRouteRule(rules);
      ...
      if (config.disallowNonIndexableRoutes) {
        Object.entries(nuxt.options.routeRules || {}).forEach(([route, rules]) => {
          ...
          const robotsRule = nitro_js.normaliseRobotsRouteRule(rules);
      ...
      if (resolvedAutoI18n && resolvedAutoI18n.locales && resolvedAutoI18n.strategy !== "no_prefix") {
        ...
        for (const group of config.groups.filter((g) => !g._skipI18n)) {
          group.allow = util_js.asArray(group.allow || []).map((path) => splitPathForI18nLocales(path, i18n)).flat();
          group.disallow = util_js.asArray(group.disallow || []).map((path) => splitPathForI18nLocales(path, i18n)).flat();
        }
      }
      const groups = config.groups.map(util_js.normalizeGroup);
      const pathsToCheck = [
